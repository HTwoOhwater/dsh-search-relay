	...
	factory: (require) => {
		...
		var WebSearchCardController = class {
			...
			*/
			constructor(scope, ctx) {
				this.scope = scope;
				this.ctx = ctx;
				this.form = new CardForm(scope, [textField("baseURL"), textField("model"), numberField("maxUses")], [{
					field: API_KEY_FIELD,
		...
		*/
		...
		const en = {
			...
			webSearchModelHint: "Anthropic-format model name. Leave blank to use the provider default.",
		...
		const zh = {
			...
			webSearchModel: "模型",
			webSearchModelHint: "Anthropic 格式的模型名称。留空则使用提供方默认模型。",
