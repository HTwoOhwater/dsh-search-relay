...
- **Structured blocks only.** DeepSeek runs the search server-side and returns structured `web_search_tool_result` blocks; the provider parses those blocks and never scrapes URLs out of model prose. A relay that merely forwards the `web_search` tool definition answers with `tool_use` instead of result blocks; the provider detects that and executes the search itself against Bing, returning the same structured `sources[]`. A response with neither result blocks nor a `web_search` tool call still throws `WEB_PROVIDER_ERROR` instead of degrading.
